...
export default function CategoryHub({ 
  ...
  return (
    <div id={`${categorySlug}-hub-page`} className="bg-[#FAFCFB] min-h-screen text-[#1A1A1A] font-sans antialiased">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {filteredProducts.map((p) => (
              <motion.div
                ...
                className={`group cursor-pointer bg-white rounded-2xl overflow-hidden border shadow-xs hover:shadow-lg transition-all duration-300 flex flex-col justify-between ${
                  p.isListingSlot ? 'border-amber-300 hover:border-[#FF5A36] ring-1 ring-amber-200' : 'border-slate-200 hover:border-[#FF5A36]'
                }`}
              >
                <div>
                  {/* Image container */}
                  <div className="aspect-[4/3] bg-slate-100 relative overflow-hidden">
                    ...
                    <button
                      ...
                      className="absolute top-3 right-3 bg-white/95 hover:bg-white text-[#0B3D3B] hover:text-[#FF5A36] px-2.5 py-1 rounded-lg text-xs font-mono font-bold shadow-md transition-all cursor-pointer flex items-center gap-1"
                      title="Edit listing details & images"
                    >
                  ...
                  {/* Listing slot highlight badge */}
                  {p.isListingSlot && (
                    <div className="bg-amber-50 border-b border-amber-200 px-4 py-1.5 flex items-center gap-1.5 text-[10px] font-mono font-bold text-amber-800">
                      <ImageIcon className="w-3 h-3 text-[#FF5A36] shrink-0" />
                      <span className="truncate">Ready Slot: Click to add photos &amp; details</span>
                    </div>
                  )}
                ...
                <div className="p-5 pt-0">
                  <div className={`w-full py-2.5 px-4 rounded-xl font-bold text-xs uppercase tracking-wider inline-flex items-center justify-between transition-all ${
                    p.isListingSlot
                      ? 'bg-[#FF5A36] text-white hover:bg-[#e44e2b]'
                      : 'bg-[#D9F0EC] group-hover:bg-[#0B3D3B] text-[#0B3D3B] group-hover:text-white'
                  }`}>
                    <span>{p.isListingSlot ? '+ Add Images & Details' : 'View Specifications'}</span>
            ...
            {/* Interactive New Listing Window Card */}
            <motion.div
              initial={{ opacity: 0, y: 15 }}
              animate={{ opacity: 1, y: 0 }}
              onClick={openNewListingModal}
              className="group cursor-pointer bg-slate-50 hover:bg-[#D9F0EC]/25 rounded-2xl border-2 border-dashed border-slate-300 hover:border-[#FF5A36] p-6 flex flex-col items-center justify-center text-center space-y-4 transition-all min-h-[360px]"
            >
              <div className="w-14 h-14 rounded-2xl bg-white border border-slate-200 group-hover:border-[#FF5A36] flex items-center justify-center text-[#FF5A36] shadow-xs group-hover:scale-110 transition-transform">
              ...
              <div className="space-y-1.5">
                ...
                <h3 className="font-display font-bold text-base text-[#0B3D3B]">
                  + Add More Images &amp; Details
                </h3>
                <p className="text-slate-500 text-xs max-w-xs leading-relaxed">
                  Open a new listing window to upload product photos, specify materials, sizes, and technical features.
                </p>
              </div>
              <span className="text-xs font-mono font-bold text-[#0B3D3B] bg-white group-hover:bg-[#0B3D3B] group-hover:text-white px-4 py-2 rounded-xl border border-slate-200 group-hover:border-transparent transition-all shadow-xs">
                Open Listing Window
              </span>
            </motion.div>
          </div>
          ...
          <div className="text-center py-16 bg-white rounded-2xl border border-slate-200 space-y-3">
            ...
            <div className="flex items-center justify-center gap-3 pt-2">
              <button
                ...
                className="bg-[#0B3D3B] text-white px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider cursor-pointer"
              >
                Reset Filters
              </button>
              <button
                onClick={openNewListingModal}
                className="bg-[#FF5A36] text-white px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider cursor-pointer"
              >
                + Add Listing Window
              </button>
            </div>
      ...
      {/* Listing Window Modal for adding images and specifications */}
      <ListingModal
